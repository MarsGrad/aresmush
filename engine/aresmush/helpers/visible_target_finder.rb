module AresMUSH
  ...
  class VisibleTargetFinder
    ...
    def self.find(name, viewer)
      ...
      chars = viewer_room.characters.select { |c| c.name_upcase == name || c.admin_name_upcase == name }
      contents = [chars, exits, rooms].flatten(1)

      if (contents.count == 0)
        exits = viewer_room.exits.select { |e| e.name_upcase.start_with?(name) }
        chars = viewer_room.characters.select { |c| c.name_upcase.start_with?(name) || c.admin_name_upcase.start_with?(name) }
        rooms = viewer_room.name_upcase == name ? [ viewer_room ] : []
        contents = [chars, exits, rooms].flatten(1)
      end
